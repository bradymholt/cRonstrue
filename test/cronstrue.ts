...
describe("Cronstrue", function () {
    ...
    describe("ranges", function () {
        ...
        it("1 1,3-4 * * *", function () {
            assert.equal(construe.toString(this.test.title), "At 1 minutes past the hour, at 01:00 AM and 03:00 AM through 04:59 AM");
        });
    });
    ...
    describe("dayOfWeekStartIndexZero=false", function () {
        it("23 12 * * 1#2", function () {
            assert.equal(construe.toString(this.test.title, { dayOfWeekStartIndexZero: false }), "At 12:23 PM, on the second Sunday of the month");
        });

        it("* * * ? * 2-6/2", function () {
            assert.equal(construe.toString(this.test.title, { dayOfWeekStartIndexZero: false }), "Every second, every 2 days of the week, Monday through Friday");
        });

        it("* * * ? * 7", function () {
            assert.equal(construe.toString(this.test.title, { dayOfWeekStartIndexZero: false }), "Every second, only on Saturday");
        });

        it("* * * ? * 1,2,3,4,5", function () {
            assert.equal(construe.toString(this.test.title, { dayOfWeekStartIndexZero: false }), "Every second, only on Sunday, Monday, Tuesday, Wednesday, and Thursday");
        });
    });

    describe("non-trival expressions", function () {
        it("*/5 15 * * MON-FRI", function () {
            assert.equal(construe.toString(this.test.title), "Every 5 minutes, at 03:00 PM, Monday through Friday");
        });
        ...
        it("5-10 30-35 10-12 * * *", function () {
            assert.equal(construe.toString(this.test.title), "Seconds 5 through 10 past the minute, minutes 30 through 35 past the hour, between 10:00 AM and 12:59 PM");
        });
        ...
        it("2-59/3 1,9,22 11-26 1-6 ?", function () {
            assert.equal(construe.toString(this.test.title), "Every 3 minutes, minutes 2 through 59 past the hour, at 01:00 AM, 09:00 AM, and 10:00 PM, between day 11 and 26 of the month, January through June");
        });
        ...
        it("0 0/30 8-9 5,20 * ?", function () {
            assert.equal(construe.toString(this.test.title), "Every 30 minutes, between 08:00 AM and 09:59 AM, on day 5 and 20 of the month");
        });
        ...
        it("0 25 7-19/8 ? * *", function () {
            assert.equal(construe.toString(this.test.title), "At 25 minutes past the hour, every 8 hours, between 07:00 AM and 07:59 PM");
        });

        it("0 25 7-20/13 ? * *", function () {
            assert.equal(construe.toString(this.test.title), "At 25 minutes past the hour, every 13 hours, between 07:00 AM and 08:59 PM");
        });
        ...
        it("0 15 6 1 1 ? 1/2", function () {
            assert.equal(construe.toString(this.test.title), "At 06:15 AM, on day 1 of the month, only in January, every 2 years");
        });
        ...
        it("0 5 7 ? * 2/3 *", function () {
            assert.equal(construe.toString(this.test.title), "At 07:05 AM, every 3 days of the week, Tuesday through Saturday");
        });
        ...
        it("0 0 0 1,2,3 * WED,FRI", function () {
            assert.equal(construe.toString(this.test.title), "At 12:00 AM, on day 1, 2, and 3 of the month, only on Wednesday and Friday");
        });

        it("0 2,16 1,8,15,22 * 1,2", function () {
            assert.equal(construe.toString(this.test.title), "At 02:00 AM and 04:00 PM, on day 1, 8, 15, and 22 of the month, only on Monday and Tuesday");
        });
    });
